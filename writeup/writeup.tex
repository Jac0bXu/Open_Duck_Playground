...
\title{Lessons from Sim-to-Real on a 15-DOF Bipedal Robot:\\Hardware Pitfalls, Reward Overfitting, and the Failure of BC Diffusion Policy}
...
\paragraph{Behavior cloning limitations.} The compounding error problem in BC was formalized by \citet{ross2011dagger}, who showed that BC policies can degrade quadratically with horizon due to distribution shift. This is well-understood in the literature but has been primarily studied in manipulation and driving; our work demonstrates how severely it manifests in bipedal locomotion, where even small errors lead to unrecoverable falls.
...
\section{Hardware Assembly Mistakes}
...
It is unfortunate that constraints prevented going further on this project at this point. However, this work was conceived as a warm-up for a larger effort: a planned collaboration on hierarchical multi-agent policies for a team of four Open Duck robots, where the lessons learned here---about sim-to-real gaps, hardware pitfalls, and policy-class selection---will directly inform the multi-agent setting.

More broadly, the intuitions built through this project transfer across embodiments. Future work will apply these lessons on different platforms, including quadrupedal locomotion on the Boston Dynamics Spot and bimanual manipulation on Franka arms. The core insights are embodiment-agnostic: system identification matters more than domain randomization, closed-loop feedback is essential for unstable systems, and honest failure analysis accelerates learning more than selective success reporting.
